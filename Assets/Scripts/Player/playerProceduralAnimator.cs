...
public class playerProceduralAnimator : MonoBehaviour
{
    ...
    [SerializeField] float tuckedLegsOffset = 3.0f;
    ...
    private void RotateHead()
    {
        ...
        if (physicsGrabber.chargingThrow) 
        {
        ...
        if (stepLeftFoot && leftFootLerp >= 1f && stepWaitTimer >= stepWaitTime)
        {
            if (Vector3.Distance(leftFoot.position, leftLegRenderer.transform.position) > stepDistance)
            {
                ...
                else
                {
                    //in the air
                    leftFootPreviousPosition = leftFoot.position;
                    leftFootTargetPosition = leftLegRenderer.transform.position + (transform.up * tuckedLegsOffset * 2.0f);
                    leftFootLerp = 0f;
                    stepLeftFoot = false;
                    stepWaitTimer = 0f;
                }
        ...
        else if (!stepLeftFoot && rightFootLerp >= 1f && stepWaitTimer >= stepWaitTime)
        {
            if (Vector3.Distance(rightFoot.position, rightLegRenderer.transform.position) > stepDistance)
            {
                ...
                else
                {
                    ...
                    rightFootPreviousPosition = rightFoot.position;
                    rightFootTargetPosition = rightLegRenderer.transform.position + (transform.up * tuckedLegsOffset); 
                    rightFootLerp = 0f;
                    stepLeftFoot = true;
                    stepWaitTimer = 0f;
                }
    ...
    private void UpdateArmTargetPositions() 
    {
        ...
        else if (physicsGrabber.grabbing)
        ...
        {
            leftHand.position = Vector3.Lerp(leftHand.position, leftHandTarget.position, armLerpSpeed * 2.0f);
            rightHand.position = Vector3.Lerp(rightHand.position, rightHandTarget.position, armLerpSpeed * 2.0f);
            Debug.Log("default");
    ...
    private void TuckLegs() 
    {
        ...
        Vector3 tuckOffset = transform.up * tuckedLegsOffset;
