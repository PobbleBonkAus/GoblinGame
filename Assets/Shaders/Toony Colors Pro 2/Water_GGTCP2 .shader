	...
	SubShader
	{
		...
		#pragma shader_feature_local _ _ALPHAPREMULTIPLY_ON
			...
			) : SV_Target
			{
				...
				// Shader Properties Sampling
				float4 __albedo = ( TCP2_TEX2D_SAMPLE(_BaseMap, _BaseMap, input.pack1.xy + frac(_Time.yy * _BaseMap_SC.xy)).rgba );
		...
		HLSLINCLUDE
			...
			) : SV_TARGET
			{
				...
				// Shader Properties Sampling
				float4 __albedo = ( TCP2_TEX2D_SAMPLE(_BaseMap, _BaseMap, input.pack2.xy + frac(_Time.yy * _BaseMap_SC.xy)).rgba );
				float4 __mainColor = ( _BaseColor.rgba );
				float __alpha = ( __albedo.a * __mainColor.a );
